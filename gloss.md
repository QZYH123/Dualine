...
dualine 只做一件事：让人读完一篇短文，觉得自己摸清了一个项目。屏幕上永远是[同一套对照](src/App.tsx#L68-L83)——有项目就进 `Reader`，没有就停在一句说明。它不生成散文，不挑仓库，不把两栏叠起来凑合窄屏。
...
磁盘上的笔记一改，页面不会自己抢焦点。`App` [每隔两秒半问一次 `rev`](src/App.tsx#L39-L60)；对不上就亮一颗「已更新」，点了才重新载入。阅读位置是你的，不是文件监视器的。

## 笔记的结构

`gloss.md` 不是博客，是一份对着代码的文书。模型很窄：一篇文档、若干章、每章里一段段 *passage*，每段面对一个 [`CodeRef`](src/lib/gloss.ts#L11-L15)——文件加上 1-based 的闭区间。

解析按空行切段。[`#` 是标题，`##` 是章](src/lib/gloss.ts#L227-L245)；标题下、第一章前的段落是导语，不驱动代码栏。每一段的面对范围是：段首的 `@[path#L1-L9]`，否则段内第一枚锚点，否则继承上一段。[`parseRef`](src/lib/gloss.ts#L67-L73) 认 `path#L18-L59` 这种写法，单行 `#L15` 也可以，写反了会排好。

普通 Markdown 链接如果不是这种靶，会原样留在字里——所以不要用它做站外跳转。

继承是事后补的。[`resolveRefs`](src/lib/gloss.ts#L269-L277) 顺着全部段落走一遍，把空的 `ref` 填成「目前面对的那一段」。跨章也继承，所以连续几段讲同一段代码，不必反复贴同一条链接。
...
下划线短语是细的绳子。悬停只点亮那几行；点击则[切换固定](src/reader/Reader.tsx#L106-L108)——代码停住，散文还可以往下读。[`Esc` 全局释放](src/reader/Reader.tsx#L112-L118)，代码栏标题上那颗按钮也是。

锚点在散文里是一条 [`<a>`](src/reader/Prose.tsx#L110-L123)：`href` 只是占位，点击被拦住，真正发生的是 pin。标题属性写出文件和行号，悬停、聚焦都会把对面的行洗成朱。
...
点这样一行，[`jumpToAnchor`](src/reader/Reader.tsx#L127-L164) 把解释它的段落送到阅读线；滚动停稳之后再 pin，代码才稳住，好让你对着读。拖过一行选中文字，不会误针定。
...
窄窗口不硬叠。[低于约 1000px 只说需要更宽的窗口](src/reader/Reader.tsx#L249-L255)，对照仪式保持诚实。
